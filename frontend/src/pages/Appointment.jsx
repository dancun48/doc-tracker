...
const Appointment = () => {
  ...
  const getAvailabeSlot = async () => {
    ...
    for(let i = 0; i < 7; i++){
      ...
      // settings and time of the day with index
  ...
  const bookAppointment = async () => {
    if (!token) {
      toast.warn('You MUST login to book an appontment');
  ...
  useEffect(() => { getAvailabeSlot() }, [docInfo]);
    ...
    <div>
      ...
      <div className='flex flex-col sm:flex-row gap-4'>
        ...
        <div className='flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0'>
          ...
          {/*----doctor's about info---- */}
          <div>
            <p className='flex items-center gap-1 text-sm font-medium tex-gray-900 mt-3'>
              About 
          ...
          <p className='text-gray-500 font-medium mt-4'>
            Appointment fee: 
            <span className='text-gray-600'> {currencySymbol} {docInfo.fees}.00</span>
          </p>
        </div>
      </div>
      {/*--------booking slots---------*/}
